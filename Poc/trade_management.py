...
async def close_trades_by_symbol(symbol):
    ...
    if not open_positions:
        print(f"No open positions for {symbol}. {open_positions}")
        return
    ...
    for position in open_positions:
        ...
        if result.retcode == mt5.TRADE_RETCODE_DONE:
            message = f"Trade closed successfully for {symbol}. Result: {result}"
            # await send_discord_message_type(message, "trade", True)
            print("Order close result:", result)
        elif result.retcode == mt5.TRADE_RETCODE_REJECT:
            message = f"Trade close rejected for {symbol}: {result.comment}"
            # await send_discord_message_type(message, "error", True)
            print(message)
        else:
            # Handle other error conditions if needed
            message = f"Trade close failed for {symbol}. Retcode: {result.retcode} Comment: {result.comment}"
            # await send_discord_message_type(message, "error", True)
            print(message)